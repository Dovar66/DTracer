...
class TraceClassVisitor extends ClassVisitor {
    ...
    TraceClassVisitor(final ClassVisitor classVisitor) {
    ...
    void visit(int version, int access, String name, String signature, String superName, String[] interfaces) {
        ...
        // 打印调试信息
        LogUtils.info("\n||---开始扫描类：${mClassName}")
        LogUtils.info("||---类详情：version=${version};\taccess=${TransformUtils.accCode2String(access)};\tname=${name};\tsignature=${signature};\tsuperName=${superName};\tinterfaces=${interfaces.toArrayString()}")
    ...
    MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        ...
        return new TraceMethodVisitor(Opcodes.ASM6, methodVisitor, name, mClassName)
    }
    ...
    @Override
    void visitEnd() {
        LogUtils.info("||---结束扫描类：${mClassName}\n")
        super.visitEnd()
    }
}